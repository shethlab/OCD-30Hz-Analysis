classdef Violin < handle
    ...
    methods
        function obj = Violin(data, pos,left, varargin)
            ...
            jitter = (max(jitterstrength)+0.1)*ones(length(data),1);%abs(2*(rand(size(data))-0.5));
            if left
                jitter = -jitter;
            end
            obj.ScatterPlot = ...
                scatter(pos + jitter+jitterstrength.*(2*(rand(size(data))-0.5))/10, data, 'filled');
            
            
            % plot the violin
            if ~left
            obj.ViolinPlot =  ... % plot color will be overwritten later
                fill([pos+density*width pos*ones(1,length(value))], ...
                     [value value(end:-1:1)], [1 1 1]);
            else
                obj.ViolinPlot =  ... % plot color will be overwritten later
                fill([pos*ones(1,length(value)) pos-density(end:-1:1)*width], ...
                     [value value(end:-1:1)], [1 1 1]);
            end
...
obj.BoxPlot = ... % plot color will be overwritten later
            ...
            if left
            obj.MedianPlot = scatter(pos-args.BoxWidth/2, quartiles(2), [], [1 1 1], 'filled');
            else
                            obj.MedianPlot = scatter(pos+args.BoxWidth/2, quartiles(2), [], [1 1 1], 'filled');
            ...
            xlim([pos-max(density*width)-0.3 pos+max(density*width)+0.3]);
        end
