...
opt.WinLength = 2;                  % STFT window length
opt.WinOverlap = 50;                % Overlap between sliding windows (in %)
opt.WinAverage = 1;                 % Number of sliding windows averaged by time point
% specparam opts
opt.freq_range          = [1 60];
opt.peak_width_limits   = [0.5 2];
opt.max_peaks           = 1;
...
newversion = 0; % 1 is legacy, do not use
...
for chan = 1:nChan
    ...
    spec = log10(squeeze(TF(chan,:,:)));
    spec(isinf(spec)) = 0;% extract log spectra for a given channel
    % Iterate across time
...
if params(2) <0.1
    ys = expo_nk_function(freqs, params);
else
    ys = params(1) + log10(abs(params(2))^params(3) +1^params(3))- log10(abs(params(2))^params(3) +freqs.^params(3));
end
end
...
ys = params(1) + log10(1^params(3))- log10(freqs.^params(3));
...
switch (aperiodic_mode)
    ...
    case 'knee'
        lb = [-Inf 0.1 0];
        ub = [Inf 250 Inf];
        aperiodic_params = fmincon(@error_expo_function, guess_vec,[],[],[],[], ...
            lb,ub,[], options, freqs_ignore, spectrum_ignore);
% %         if aperiodic_params(2)<0.1 | aperiodic_params(2)>250
% %             aperiodic_params = fminsearch(@error_expo_nk_function, guess_vec, options, freqs_ignore, spectrum_ignore);
% %             aperiodic_params = [aperiodic_params(1) 0 aperiodic_params(2)];
% % 
% %         end
% %         if aperiodic_params(3)<0
% %             aperiodic_params = fminsearch(@error_expo_nk_function, guess_vec, options, freqs_ignore, spectrum_ignore);
% %             aperiodic_params = [aperiodic_params(1) 0 aperiodic_params(2)];
% %         end
end
